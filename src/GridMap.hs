...
prop_gridNeighbors grid = all (\t -> neighbors grid Map.! (place t) == map (\p -> place (getTile grid (p `div` cols grid) (p `mod` cols grid))) (adjacents (rows grid) (cols grid) (place t))) (tiles grid)
